...
import org.apache.spark.metrics.sink.DatadogReporter._
...
/**
  ...
  */
private[spark] object DatadogReporter {
  val COUNTER = "c"
  val GAUGE = "g"
  val TIMER = "ms"
  val Set = "s"

  val DefaultStatsDPort = 8125

  val EmptyString = ""
  val ApplicationTagName = "appplication"
  val ExecutorTagName = "executor"
  val PrefixTagName = "prefix"
  val MetricTagName = "metric"
  val patternExecutor = new Regex("""^(\p{InBasicLatin}+?)\.(application_\p{InBasicLatin}+?)\.(\d+?)\.(.+)""",
                                  PrefixTagName,
                                  ApplicationTagName,
                                  ExecutorTagName,
                                  MetricTagName)
  val pattern = new Regex("""^(\p{InBasicLatin}+?)\.(application_\p{InBasicLatin}+?)\.(.+)""",
                          PrefixTagName,
                          ApplicationTagName,
                          MetricTagName)
}

private[spark] class DatadogReporter(registry: MetricRegistry,
                                     host: String = "127.0.0.1",
                                     port: Int = DefaultStatsDPort,
                                     prefix: String = "",
                                     filter: MetricFilter = MetricFilter.ALL,
                                     rateUnit: TimeUnit = TimeUnit.SECONDS,
                                     durationUnit: TimeUnit = TimeUnit.MILLISECONDS)
    extends ScheduledReporter(registry, "datadog-reporter", filter, rateUnit, durationUnit)
    with Logging {
  ...
  override def report(gauges: util.SortedMap[String, Gauge[_]],
    ...
    Try(new DatagramSocket) match {
      case Failure(ioe: IOException) =>
        logWarning("StatsD datagram socket construction failed",
      ...
      case Success(s) =>
        implicit val socket: DatagramSocket = s
        ...
        Try {
          ...
          case ioe: IOException =>
            logDebug(s"Unable to send packets to StatsD",
                     NetUtils.wrapException(address.getHostString, address.getPort, localAddress, localPort, ioe))
  ...
  private def reportHistogram(name: String, histogram: Histogram)(implicit socket: DatagramSocket): Unit = {
  ...
  private def send(name: String, value: String, metricType: String)(implicit socket: DatagramSocket): Unit = {
    val bytes = sanitizeWithTags(name, value, metricType).getBytes(UTF_8)
  ...
  // https://github.com/apache/spark/blob/master/core/src/main/scala/org/apache/spark/metrics/MetricsSystem.scala#L138
  private def sanitizeWithTags(name: String, value: String, metricType: String): String = {

    val patternExecutorMatched = patternExecutor.findFirstMatchIn(name)
    patternExecutorMatched match {

      case Some(patternExecutorResult) =>
        val application = patternExecutorResult.group(ApplicationTagName)
        val executor = patternExecutorResult.group(ExecutorTagName)
        val sanitizedName = name
          .replaceFirst(s".$application", EmptyString)
          .replaceFirst(s".$executor", EmptyString)
        val tags = tagString(s"$ApplicationTagName:$application", s"$ExecutorTagName:$executor")
        sanitize(s"$sanitizedName:$value|$metricType$tags")
      ...
      case _ =>
        ...
        matched match {
          ...
          case Some(result) =>
            val application = result.group(ApplicationTagName)
            val sanitizedName = name.replaceFirst(s".$application", EmptyString)
            val tags = tagString(s"$ApplicationTagName:$application")
            sanitize(s"$sanitizedName:$value|$metricType$tags")

          case _ =>
            sanitize(s"$name:$value|$metricType")
        }
    }
  }
  ...
  /**
    ...
    */
  private def tagString(tags: String*): String = {
    if (tags.isEmpty) {
      ""
    } else {
